...
**남은 것.** 초과 409건 / 1,246B 를 사람이 줄이는 일 자체. 메뉴·이름·필드
...
목표: kernel.bin 텍스트 409건이 원래 자리보다 길어 못 들어갔다. 이걸 사람이
...
번역은 1,320건 전부 끝났다(`work/text/kernel-text-ko.json`). 그중 **911건은
들어갔고 409건은 원래 자리보다 길어 못 들어갔다.**

```
#39 아이템 설명  112건    #32 마법      54건    #34 어빌리티  47건
#31 전투커맨드    31건    #55 전투 결과  27건    #33          19건
#42 17건  #44 14건  #46 13건  #47 11건  #54 10건  #38 9건  … 그 밖
```
...
- ~~kernel 음절 104자가 뱅크1~~ **끝났다(배치 v14~v16).** 생성기가 kernel
  음절도 뱅크0 에 우선 배치한다. `닉`(피닉스의 꼬리)이 정상으로 나온다.
  대가로 초과가 462 -> 509건까지 늘었다가, v16 의 `--relieve` 로 409건이 됐다
