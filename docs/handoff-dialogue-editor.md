...
**남은 것.** 초과 23건 / 27B (2026-08-09 축약 뒤). 메뉴·이름·필드
...
목표: kernel.bin 텍스트 23건이 원래 자리보다 길어 아직 못 들어간다(전 409건).
남은 것은 전부 음역 고유명사이고 1바이트 구간이 포화라 배치로도 못 푼다.
필드 대사 편집 기능은 그대로 둔다.
...
번역은 1,320건 전부 끝났다(`work/text/kernel-text-ko.json`). **1,297건이
들어가고 23건이 남았다** (2026-08-09 축약 작업 뒤).
