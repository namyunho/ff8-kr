...
**`Z` 패킷은 `OK` 를 돌려주고 아무 일도 하지 않는다.** 초당 60번 실행되는 예외
벡터(`0x80000080`)에 걸어도 3초 안에 한 번도 걸리지 않는다. 물리 주소로도,
소프트·하드 둘 다 마찬가지다.

**응답을 지원의 증거로 삼으면 안 된다.** 이 저장소는 한 번 그렇게 판정했다가
정정했다 — `OK` 다섯 개를 보고 "워치포인트 전부 지원" 이라고 적었다. 도구의
기능은 **동작으로** 확인한다.
