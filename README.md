...
| 번역문의 서로 다른 음절 수 | **1,080** — 882칸을 넘지만 서브셋으로 간다 |
...
기계 초벌번역(실제 대사 99.6%)을 세어 M5 의 마지막 전제를 판정했다.
...
**882칸을 넘지만 뱅크0 하나로 간다.** 한국어 음절 빈도가 극단적으로 쏠려 있어
잘려 나가는 것은 거의 안 쓰이는 음절이고, 고칠 자리는
`check_translation.py --layout` 이 짚는다. 자세한 내용은
