...
| 번역문 바이트 수용량 | 측정 완료 — 배치를 정하면 들어간다 |
| 런타임(에뮬레이터) 검증 | **미착수** |
...
목록 1,003엔트리를 전수로 훑으면 **DAT 302개, MSD 메시지 9,442건**이다. DAT 는
목록만으로 가려지지 않고, 해제한 뒤 포인터 12개가 단조 증가하고 `ptr[11]` 이
해제 크기와 맞물리는지로 판정한다.
...
|---|---|
| 손대지 않고 재압축만 | 2 / 302 |
| 한글을 전부 224 뒤에 (최악) | 27 / 302 |
| **빈도 상위 224자를 1바이트 자리에** | **0 / 302** |

본문이 심하게 쏠려 있어(가장 잦은 224자가 본문의 93.00%를 덮는다) 배치를 정하면
들어간다. **빈도 상위 224자를 인덱스 0..223 에 배치한다**가 한글 폰트의 설계
제약이다. 다만 이 계산은 일본어 원문의 쏠림을 한국어의 대리 지표로 쓴 것이라
실제 값은 번역 초안에 같은 도구를 다시 돌려 재야 한다.
