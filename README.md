...
| **4중 인터리브** | **실기 확인 완료** — 882 → **1,764칸**. 여유 663 |
...
**부호·숫자·라틴 63칸을 더해도 1,180 안팎**이다. 4중 인터리브의 1,764칸이면
여유가 580 이상 남는다.
...
4중 인터리브는 **게임이 이미 우리 것으로 인정한 자리의 밀도만** 올린다. 같은
1,764칸을 다른 자원으로 얻는다.

### 팔레트

평면이 넷이니 팔레트도 테마당 넷이 필요하다. 실측으로 `(288, 240~255)` 16줄이
**완전히 비어 있어** 그 아래에 얹었다.
...
# 1,764칸 배치와 4중 인터리브 폰트
python3 scripts/count_korean_syllables.py work/translate \
  --layout work/hangul-layout-4plane.json --banks 2
...
python3 scripts/patch_disc.py --apply work/apply-plan.json \
  --layout work/hangul-layout-4plane.json
